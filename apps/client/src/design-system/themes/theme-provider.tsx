...
// This external store deliberately starts from the SSR default. useSyncExternalStore
// uses the same server snapshot during hydration, then this store is updated from
// localStorage after the first matching client render has completed.
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  // Reading localStorage during initialization makes the first client render
  // differ from SSR when a visitor previously selected dark mode. Always start
  // from the registry default, then restore the persisted choice after React
  // has hydrated the identical server markup.
